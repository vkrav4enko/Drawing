...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        _startPoint = CGPointMake(500, 400);
        _anglePoint = CGPointMake(100, 300);
        _endPoint = CGPointMake(500, 200);
        [self setupLayer];
    }
    return self;
}
...
- (void)setupLayer
{
    ...
    [path addLineToPoint:_anglePoint];
...
- (BOOL) checkPoint: (CGPoint) point withinRadius:(float)r
{
    ...
    dx = point.x - _anglePoint.x;
    dy = point.y - _anglePoint.y;
    if(sqrtf(dx*dx + dy*dy)<=r)
    {
        _pointToChange = &_anglePoint;
        return YES;
    }
